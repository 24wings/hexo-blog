...
! function () {
    ...
    b[0] = {
        ...
        value: function (a, c, d) {
            ...
            h.version = "1.4.37", h.registerInitHook = function (a) {
                g.push(a)
            }, d.exports = h
        }
    }, b[20] = {
        ...
        value: function (a, c, d) {
            ...
            function g(a, b, c) {
                function d(a, b) {
                    a.save(), a.fillStyle = b, a.fillRect(0, 0, n.width, n.height), a.restore()
                }
                ...
                function l() {
                    ...
                    return e(a).then(function (a) {
                        return g(o, a.element, v, w, a.width, a.height), j(y)
                    }).then(function (a) {
                        for (var b = 0; b < a.length; b++) g(o, a[b].element, a[b].x, a[b].y, a[b].width, a[b].height);
                        return k.revokeObjectURL(x), document.body.appendChild(n), i(n)
                    }, function (a) {
                        return alert("脑图的节点中包含跨域图片，导出的 png 中节点图片不显示，你可以替换掉这些跨域的图片并重试。"), k.revokeObjectURL(x), document.body.appendChild(n), i(n)
                    })
                }
                var m = kityminder.Promise,
                    n = document.createElement("canvas"),
                    o = n.getContext("2d"),
                    p = b.getStyle("background").toString(),
                    q = /url\(\"(.+)\"\)/.exec(p),
                    r = h.Color.parse(p),
                    s = f(b),
                    t = c && c.width && c.width > s.width ? c.width : s.width,
                    u = c && c.height && c.height > s.height ? c.height : s.height,
                    v = c && c.width && c.width > s.width ? (c.width - s.width) / 2 : 0,
                    w = c && c.height && c.height > s.height ? (c.height - s.height) / 2 : 0,
                    x = s.dataUrl,
                    y = s.imagesInfo,
                    z = 20;
                if (n.width = t + 2 * z, n.height = u + 2 * z, q) {
                    var A = {
                        url: q[1]
                    };
                    return e(A).then(function (a) {
                        return d(o, o.createPattern(a.element, "repeat")), l()
                    })
                }
                return d(o, r.toString()), l()
            }
            var h = b.r(17),
                i = b.r(12),
                j = b.r(25),
